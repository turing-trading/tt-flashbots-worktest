...
@pytest.mark.asyncio
async def test_composite_key_uniqueness(async_session: AsyncSession) -> None:
...
async def test_slot_to_block_consistency(
    ...
    assert len(inconsistent) == 0, (
        f"Found {len(inconsistent)} slots with multiple different block numbers: "
        f"{inconsistent[:10]}"
    )


@pytest.mark.asyncio
async def test_relay_coverage(async_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_builder_pubkey_format(
    async_session: AsyncSession, max_violations: int
) -> None:
...
@pytest.mark.asyncio
async def test_value_field_non_negative(async_session: AsyncSession) -> None:
...
async def test_gas_values_consistency(
    ...
    assert len(violations) == 0, (
        f"Found {len(violations)} relay payloads where gas_used > gas_limit: "
        f"{violations[:10]}"
    )


@pytest.mark.asyncio
async def test_num_tx_non_negative(async_session: AsyncSession) -> None:
...
async def test_relay_payload_references_valid_block(
    ...
    assert len(orphaned) == 0, (
        f"Found {len(orphaned)} relay payloads referencing non-existent blocks: "
        f"{orphaned[:10]}"
    )
